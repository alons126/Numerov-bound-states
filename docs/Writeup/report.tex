...
\begin{document}
    ...
    \section{Computational approach}
        ...
        \subsection{Shooting and root finding}
            ...
            \end{align}
            The inward formulation is more specialized: it assumes that the chosen truncation point lies in a forbidden tail where the physical solution is locally exponential, so it is used here for smooth confining infinite-domain problems rather than as a universal replacement for the outward method. Eigenvalues correspond to the roots of $M(E)=0$, which are located by bracketing sign changes and then refined numerically. To make the procedure transparent, the report now uses two complementary diagnostics: a global raw-mismatch scan over the full energy interval, where only the final root estimates are marked, and per-root zoom plots that restrict the horizontal axis to one initial bracket and show the full bisection history. Zero crossings identify the allowed energies. \\
