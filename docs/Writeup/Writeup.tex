...
\begin{document}
    ...
    \section{Introduction}
        ...
        The full code was written in Python and is available in \href{https://github.com/alons126/Numerov-bound-states}{this repository}. Explanations of the code structure, usage, and implementation details are also provided there in the \href{https://github.com/alons126/Numerov-bound-states/blob/main/README.md}{\texttt{README.md}}, the \href{https://github.com/alons126/Numerov-bound-states/blob/main/docs/code_walkthrough.md}{\texttt{code\_walkthrough.md}} guide, and comments throughout the source files. The implementation is modular, with separate components for the Numerov integrator, shooting method, root finding, analysis utilities, and an automated validation suite. This structure allows each numerical component to be validated independently and makes the overall workflow easier to understand.
    ...
    \section{Computational approach}
        \subsection{The Schr\"odinger equation and Numerov method}
            ...
            \psi''(x) = 2\,[V(x)-E]\psi(x)
            \end{equation}
            The wavefunction is integrated with the Numerov scheme on a uniform grid. For the symmetric bound-state potentials studied here, $V(-x)=V(x)$, so the eigenstates can be chosen to have definite parity: even states satisfy $\psi(-x)=\psi(x)$ and odd states satisfy $\psi(-x)=-\psi(x)$. That symmetry implies exact conditions at the origin, $\psi'(0)=0$ for even states and $\psi(0)=0$ for odd states, so the solver can integrate only on the half-domain $[0,x_{\max}]$ and reconstruct the other half by reflection. Eigenvalues are found by scanning for sign changes in a scalar mismatch function and then refining the resulting brackets with robust root finding; in the outward Numerov solver this is bisection followed by a few safeguarded secant-style polishing steps, while the inward-decay and RK4 solvers use straight bisection. The correctness of the numerical approach is verified through comparison with analytical solutions where available, through systematic convergence studies, and through direct automated tests of the implementation. \\
        ...
        \subsection{Shooting and root finding}
            The boundary-value eigenproblem is reformulated as a root-finding problem for a mismatch function $M(E)$. For a given trial energy $E$, the Schr\"odinger equation is integrated and a boundary mismatch is evaluated. Because the potential is symmetric, the half-domain solution is enough: once parity is chosen, the left half of the wavefunction is fixed by reflection and the unknown energy is the only quantity left to determine. The code uses two related formulations. For the infinite square well, finite square well, and quartic double well, the half-domain problem is shot outward from $x=0$ with parity-consistent startup values and the raw mismatch is the outer boundary value $M(E)=\psi_E(x_{\max})$. This outward formulation is the more general one in the project, because the startup comes from the exact parity conditions at the symmetry point and therefore applies to any symmetric potential once the half-domain reduction is valid. For the harmonic oscillator and the RK4 comparison, the code instead shoots inward from $x_{\max}$ through the decaying tail and enforces parity at the origin:
            \begin{align}
            \text{even states:}\quad M(E) = \psi'_E(0),\,\,\,\,\,\,\,\,\,\,\text{odd states:}\quad M(E) = \psi_E(0)
            \end{align}
            The inward formulation is more specialized: it assumes that the chosen truncation point lies in a forbidden tail where the physical solution is locally exponential, so it is used here for smooth confining infinite-domain problems rather than as a universal replacement for the outward method. The two formulations also differ slightly in how the brackets are polished once a sign change has been found. The outward solver bisects the wall-mismatch bracket and then performs a few safeguarded secant-style updates inside the final sign-changing interval, which improves the final raw boundary residual without giving up bracketing robustness. The inward and RK4 solvers use straight bisection on the origin-parity mismatch instead, which keeps that specialized formulation simpler and fully bracketed. Eigenvalues correspond to the roots of $M(E)=0$, which are located by bracketing sign changes and then refined numerically. To make the procedure transparent, the report now uses two complementary diagnostics: a global raw-mismatch scan over the full energy interval, where only the final root estimates are marked, and per-root zoom plots that restrict the horizontal axis to one initial bracket and show the full bisection history. Zero crossings identify the allowed energies. \\
    ...
    \section{Results}
        ...
        \subsection{Double-well potential}
            ...
            \begin{figure}[p]
                ...
                \begin{minipage}[t]{0.48\textwidth}
                    ...
                    \captionof{figure}{Grid convergence for the quartic double well. Since no exact spectrum is available, the vertical axis shows the successive-refinement surrogate $|E(h_i)-E(h_{i+1})|$ rather than $|E_{\text{numerical}}-E_{\text{exact}}|$. The curves still decrease with clear high-order behavior under grid refinement.}
                ...
                \begin{minipage}[t]{0.48\textwidth}
                    ...
                    \captionof{figure}{Dependence of the double-well energy errors on box size, measured relative to a separate larger-box reference calculation at approximately fixed grid spacing. The errors decrease rapidly as the domain is enlarged and then reach a numerical plateau.}
                ...
                \begin{minipage}[t]{0.48\textwidth}
                    ...
                    \captionof{figure}{Lowest two double-well energies together with their splitting as the parameter $b$ is varied. The splitting decreases as the barrier becomes stronger, consistent with tunneling suppression.}
        ...
        \subsection{Finite square well}
            ...
            The finite-well grid-refinement study provides a modest quantitative check in addition to these qualitative diagnostics: the first few bound-state energies converge toward the finest-grid reference as $h$ decreases, but the observed slope is noticeably lower than in the smooth benchmark cases because the discontinuous well edge dominates the error. Even so, this part of the project remains primarily an additional physical test rather than the main convergence showcase. In particular, the finite-well figures confirm that the solver distinguishes strongly bound interior states from a near-threshold state with pronounced barrier penetration. \\
    ...
    \section{Discussion}
        ...
        The finite square well provides an intermediate physical check between analytic bound-state benchmarks and more complex tunneling problems. Its low-lying states remain localized inside the well, while the highest bound state lies only about $0.218$ below the barrier and therefore shows the expected long evanescent tail outside the well. Its grid-refinement plot also illustrates an important numerical limitation: because the potential jumps at the well edge, the observed convergence order is lower than in the smooth benchmark cases even though the energies still converge correctly. This confirms that the same shooting framework handles both hard-wall-like confinement and finite barrier penetration. \\
