...
\begin{document}
    ...
    The project results also justify the choice. Once the startup formulas and boundary derivative estimates were made consistent with the method's order, the solver produced clean convergence for the smooth benchmark cases such as the infinite square well, harmonic oscillator, and quartic double well. The finite square well is more subtle because its piecewise-discontinuous wall makes the observed grid-convergence order lower than in the smooth cases, even though the energies still converge correctly. That would be harder to diagnose pedagogically with a more black-box integrator. Numerov therefore serves not only as an effective solver, but also as a useful framework for studying numerical error, parity reduction, domain truncation, and tunneling-sensitive states.
    ...
    This inward startup is not as universal as the outward parity startup at $x=0$. The outward formulation uses exact symmetry conditions and therefore applies to any symmetric potential once the half-domain reduction is valid. The inward formulation instead assumes the chosen $x_{\max}$ lies in a simple decaying tail, so in this project it is treated as a specialized tool for smooth confining infinite-domain problems rather than the default solver for every potential.

    The project also treats the final bracket refinement slightly differently in the two formulations. Outward shooting bisects the sign-changing wall mismatch and then applies a few safeguarded secant-style polishing steps inside the last bracket, because that improves the raw boundary residual without sacrificing robustness. Inward shooting keeps the refinement as straight bisection on the origin-parity mismatch, which is simpler and remains fully bracketed.
    ...
    \begin{equation}
        V_{\min}=-\frac{b^2}{4a}
    \end{equation}
    instead of the sampled grid minimum. Here the sampled grid minimum means the discrete value $\min_i V(x_i)$ computed only from the mesh points actually present on the current grid. This is important because using that sampled minimum makes the potential itself depend slightly on the grid spacing $h$, which contaminates a grid-convergence study. Using the analytic shift also moves both well bottoms to zero energy, which is a cleaner reference level for interpreting the low-lying states while leaving the well shape and tunneling splittings unchanged.
